--[[
    ...
    server (see ServerGuides_Server.lua); this file holds only pure functions
    plus the parser that the server reuses.
]]

ServerGuides = ServerGuides or {}
...
ServerGuides.CHUNK_SIZE = 8 * 1024
...
ServerGuides.MAX_UPLOAD_CHUNKS = math.ceil(ServerGuides.MAX_FILE_BYTES / ServerGuides.CHUNK_SIZE) + 1
...
ServerGuides.DEFAULT_EDIT_LEVELS = "admin;moderator;overseer;gm"
...
--- Options (page "Server Guides").
function ServerGuides.editLevelSet()
    local raw = (SandboxVars and SandboxVars.ServerGuides and SandboxVars.ServerGuides.EditAccessLevels)
    if not raw or ServerGuides.trim(raw) == "" then raw = ServerGuides.DEFAULT_EDIT_LEVELS end
    local set = {}
    for token in string.gmatch(raw, "[^;,]+") do
        local t = string.lower(ServerGuides.trim(token))
...
function ServerGuides.levelCanEdit(lvl)
    if not lvl or lvl == "" then return false end
    return ServerGuides.editLevelSet()[lvl] == true
end
...
--     options (ServerGuides.levelCanEdit).
...
function ServerGuides.isStaff(player)
    ...
    lvl = string.lower(ServerGuides.trim(lvl or ""))
    return ServerGuides.levelCanEdit(lvl)
end
...
-- (e.g. "C:\" or "/etc"). Valid paths are always relative to ServerGuides/.
-- @return true if safe; otherwise false, reason
function ServerGuides.isSafeRelativePath(path)
...
--- Full path (relative to ~/Zomboid/Lua/) of a ServerGuides file.
function ServerGuides.resolve(relPath)
    return ServerGuides.FOLDER .. "/" .. relPath
end
...
function ServerGuides.luaFileExists(luaRelPath)
...
function ServerGuides.hashNumber(str, seed)
...
function ServerGuides.hashString(str, seed)
    return string.format("%08x", ServerGuides.hashNumber(str, seed))
end
...
function ServerGuides.classifyCategory(rawName)
    local name = ServerGuides.trim(rawName)
    local isRules = false
    if string.sub(name, 1, 1) == "*" then
        isRules = true
        name = ServerGuides.trim(string.sub(name, 2))
    end
...
function ServerGuides.parseIndex(lines)
    local tree = {}
    local current = nil

    for _, raw in ipairs(lines) do
        local line = ServerGuides.trim(raw)
        ...
        elseif string.sub(line, 1, 1) == "[" and string.sub(line, -1) == "]" then
            ...
            local isRules, name = ServerGuides.classifyCategory(rawName)
            current = { cat = name, isRules = isRules, items = {} }
            table.insert(tree, current)
        else
            local key, value = string.match(line, "^(.-)=(.*)$")
            if key and current then
                key = ServerGuides.trim(key)
                value = ServerGuides.trim(value)
...
function ServerGuides.serializeIndex(tree)
    local out = {}
    table.insert(out, "# index.txt - gerado pelo editor do ServerGuides.")
    ...
    for _, cat in ipairs(tree) do
        ...
        if cat.isRules then
            ...
            local implicit = select(1, ServerGuides.classifyCategory(name))
